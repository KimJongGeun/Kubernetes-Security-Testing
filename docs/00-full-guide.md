...
Garak에서 성공했던 공격 패턴을 기반으로 3가지를 탐지한다:

- 프롬프트 인젝션 — "Ignore previous instructions" 같은 패턴
- 탈옥 시도 — "DAN", "bypass safety" 같은 패턴
- 인코딩 우회 — base64, rot13 같은 인코딩으로 숨긴 명령

위험한 입력은 LLM한테 안 보내고 바로 차단한다.

## 출력 필터

LLM 응답에서 주민번호, 카드번호 같은 민감 정보가 있으면 마스킹한다.
